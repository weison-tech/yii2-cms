<?php
namespace core\modules\file\libs;
 ...
 * @package core\modules\file\libs
...
class ImageConverter
{
    ...
    /**
     * Resize an given Image to an given Size
     *
     * Options Array:
     *      width - in px
     *      height - in px
    ...
    public static function Resize($sourceFile, $targetFile, $options = array())
    {
        ...
        //If has config external command to handle, use external command to resize the image.
        //Otherwise, use GD library to resize the image.
        if (Yii::$app->getModule('file')->settings->get('imageMagickPath')) {
            self::ResizeImageMagick($sourceFile, $targetFile, $options);
        } else {
            //Temporary store php global memory limit in variable.
            $memoryLimit = ini_get('memory_limit');
            //Dynamically allocate memory to process image
            self::allocateMemory($sourceFile);
            //Resize the image via GD library.
            self::ResizeGD($sourceFile, $targetFile, $options);
            //Reset the php global memory limit back from temporary variable.
    ...
    {
        // buffer for memory needed by other stuff (10 equal 10M)
        $buffer = 10;

        // usually for RGB 3 pixels are used, for CMYK 4 pixels (4 equal 4 Bytes)
        $bytesPerPixel = 4;
        ...
        // calc needed size for processing image dimensions in Bytes. (The 1048576 is 1024 * 1024)
        $neededMemory = floor(($width * $height * $bytesPerPixel * $tweakFactor + 1048576) / 1048576);

        //Get max memory allocation from setting.
        $maxMemoryAllocation = Yii::$app->getModule('file')->settings->get(self::SETTINGS_NAME_MAX_MEMORY_ALLOCATION);
        //If the max memory allocation is not set, use the default value witch defined this class.
        $maxMemoryAllocation = $maxMemoryAllocation ?: self::DEFAULT_MAX_ADDITIONAL_MEMORY_ALLOCATION;

        $newMemoryLimit = (Int) $memoryLimit + min($neededMemory, $maxMemoryAllocation);
        ...
        if (!$test) {
            throw new Exception("Image $sourceFile too large to be resize. Increase MAX_MEMORY_USAGE");
        }
    ...
    /**
     * Resize GD Library Implementation
     *
     * @param string $sourceFile
     * @param string $targetFile
     * @param array $options
    ...
    private static function ResizeGD($sourceFile, $targetFile, $options = array())
    {
        ...
        //The width and height size to resize.
        $width = $options['width'];
    ...
    /**
     * Set GD image's orientation via exchangeable image file info.
    ...
    {
        //Get exchangeable image file info array.
